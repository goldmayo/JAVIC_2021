...
class MyLogicAdapter(LogicAdapter):
    ...
    def initialize_vacation(self,intent):
        ...
        if (vacation.hasNext()): # 시작일 종료일 중 어떤 것 이라도 none(입력을 받지 않으면) True 둘다 입력받으면 false
            return api_frame.wrap_content(vacation.getNext())# 시작일 종료일 입력받기
        else:
            ###
    ...
    def handle_ongoing_service(self,prediction,input_statement):
        if self._service_box.has_service(): # 서비스가 실행중인 경우
            ...
            now_service = request_packet.get_service_name() # 현재 서비스의 이름 /휴가신청, 메일확인
            ...
            if intent == "no_service":# 숫자등 사용자 입력
                ...
                if intent == now_service:# 현재 서비스 재요청하는 경우
                    ...
                    if intent == 'yes': #확인
                        ...
                        return api_frame.wrap_content(now_service+" 서비스를 취소했습니다.") # 취소 메시지 전송
